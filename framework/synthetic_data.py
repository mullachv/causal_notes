...
def create_traits(N):
	return create_df_from_arr(np.random.randn(N), 'TRAITS', 'float32')
...
def load_data(data_key='data_1000x1000__x_nm'):
...
if __name__ == '__main__':
	...
	parser.add_argument('-L', "--m_low", help="Low range of SNPs (exp scale)", default=2, type=float)
	parser.add_argument('-H', "--m_hi", help="High range of SNPs (exp scale)", default=3, type=float)
	parser.add_argument('-N', "--num_m", help="SNP count set size", default=2, type=int)
	parser.add_argument('-V', "--n_low", help="Low range of individuals", default=1000, type=int)
	parser.add_argument('-W', "--n_hi", help="High range of individuals", default=5000, type=int)
	parser.add_argument('-S', "--incr_n", help="Individual count increment", default=4000, type=int)
	...
	if not (os.path.exists(data_filename)):
		for M in np.logspace(args.m_low, args.m_hi, num=args.num_m):
			...
			for N in range(args.n_low, args.n_hi, args.incr_n):
				data_key_pre = 'data_' + str(N) + 'x' + str(M) + '__'
	...
	data = load_data('data_1000x1000__x_nm')
